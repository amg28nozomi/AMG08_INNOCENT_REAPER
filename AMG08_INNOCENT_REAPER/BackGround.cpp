...
namespace inr {
  ...
  int BackGround::KeyNumber() {
    // ステージキーを取得
    auto stageKey = _game.GetModeServer()->GetModeMain()->StageKey();
    if (stageKey == stage::STAGE_0) return stage::number::SN_S;
    if (stageKey == stage::STAGE_T) return stage::number::SN_T;
    if (stageKey == stage::STAGE_1) return stage::number::SN_1;
    if (stageKey == stage::STAGE_2) return stage::number::SN_2;
    if (stageKey == stage::STAGE_2_1) return stage::number::SN_2;
    if (stageKey == stage::STAGE_2_2) return stage::number::SN_2;
    if (stageKey == stage::STAGE_3) return stage::number::SN_B;
    return -1;
  }
