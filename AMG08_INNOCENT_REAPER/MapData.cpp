...
namespace inr {
	...
	void MapDataManager::LoadStageMap(JsonMapData& jsonMapData) {
		for (auto&& j : jsonMapData) {
			...
			if (it != _maps.end()) continue;	// èàóùÇÉXÉLÉbÉv
			_maps.emplace(j.first, j.second);	// ìoò^
		}
	}

	bool MapDataManager::GetStageMap(const std::string stage, MapData& mdata) {
		// ì«Ç›éÊÇËÇΩÇ¢ÉfÅ[É^ÇÕìoò^Ç≥ÇÍÇƒÇ¢ÇÈÇ©ÅH
		auto smap = _maps.find(stage);
		if (smap == _maps.end()) {
			return false;	// ìoò^Ç≥ÇÍÇƒÇ¢Ç»Ç¢èÍçáÇÕèàóùÇî≤ÇØÇÈ
		}
		mdata = smap->second;	// éÊÇËèoÇµÇΩèÓïÒÇë„ì¸Ç∑ÇÈ
		return true;
	}
